#!/bin/bash
#SBATCH --job-name=canada_netcdf_conversion
#SBATCH --output=log/canada_netcdf_conversion.out
#SBATCH --error=log/canada_netcdf_conversion.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
echo "Starting Canada NetCDF to DGGS conversion process..."
echo "Using $NUM_CORES CPU cores for parallel processing"

# Run Canada NetCDF to DGGS conversion
echo "=========================================="
echo "Running Canada NetCDF to DGGS conversion"
echo "=========================================="
echo "Start time: $(date)"
$PYTHON_PATH scripts/netcdf_conversion/canada_netcdf_to_dggs_converter.py
EXIT_CODE=$?
echo "End time: $(date)"

if [ $EXIT_CODE -eq 0 ]; then
    echo "Canada NetCDF conversion completed successfully"
else
    echo "Canada NetCDF conversion failed with exit code $EXIT_CODE"
    exit 1
fi

echo ""
echo "Canada NetCDF to DGGS conversion completed successfully!"
