...
echo "Starting EDGAR NetCDF to DGGS conversion process..."
echo "Using $NUM_CORES CPU cores for parallel processing"

# Run EDGAR NetCDF to DGGS conversion
echo "=========================================="
echo "Running EDGAR NetCDF to DGGS conversion"
echo "=========================================="
echo "Start time: $(date)"
$PYTHON_PATH scripts/netcdf_conversion/global_edgar_netcdf_to_dggs_optimize.py
EXIT_CODE=$?
echo "End time: $(date)"

if [ $EXIT_CODE -eq 0 ]; then
    echo "EDGAR NetCDF conversion completed successfully"
else
    echo "EDGAR NetCDF conversion failed with exit code $EXIT_CODE"
    exit 1
fi

echo ""
echo "EDGAR NetCDF to DGGS conversion completed successfully!"
