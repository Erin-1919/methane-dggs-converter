...
class MexicoNetCDFToDGGSConverterAggregated:
    """
    ...
    def __init__(self, netcdf_folder, geojson_path, output_folder, area_file_path, num_cores=None):
        ...
        Args:
            netcdf_folder (str): Path to folder containing Mexico NetCDF files
        ...
        # Load IPCC lookup table
        self.log_message("Loading IPCC2006 lookup table...")
        self._load_ipcc_lookup()
        
        # Load the DGGS grid
        self.log_message("Loading Mexico DGGS grid...")
        ...
        self.log_message("Loading area data from Canada file...")
    ...
    def _load_ipcc_lookup(self):
        """Load IPCC2006 lookup table for variable mapping."""
        try:
            lookup_path = "data/lookup/mexico_netcdf_variable_lookup.csv"
            if not os.path.exists(lookup_path):
                raise FileNotFoundError(f"IPCC lookup file not found: {lookup_path}")
            
            lookup_df = pd.read_csv(lookup_path)
            self.variable_to_ipcc = dict(zip(lookup_df['variable'], lookup_df['IPCC2006']))
            self.log_message(f"Loaded IPCC lookup with {len(self.variable_to_ipcc)} variable mappings")
            
        except Exception as e:
            self.log_message(f"Error loading IPCC lookup: {e}")
            raise
    ...
    def _load_area_data(self):
        """Load area data from Canada NetCDF file and save as standalone file."""
        try:
            # Load area data from Canada file
            canada_nc = xr.open_dataset(self.area_file_path)
            self.area_data = canada_nc['area'].values
            ...
            area_output_path = os.path.join(area_npy_folder, "mexico_area_data.npy")
            np.save(area_output_path, self.area_data)
            self.log_message(f"Area data loaded and saved to: {area_output_path}")
            self.log_message(f"Area data shape: {self.area_data.shape}")
    ...
    def aggregate_variables_by_ipcc_code(self, nc_data):
        ...
        """
        self.log_message("Aggregating variables by IPCC2006 code...")
    ...
    def convert_aggregated_to_raster(self, netcdf_path):
        ...
        # Aggregate variables by IPCC2006 code
        ...
        for ipcc_code, aggregated_array in aggregated_data.items():
            self.log_message(f"  Processing aggregated IPCC code: {ipcc_code}")
            ...
            # Calculate total emissions per pixel using area data
            if self.area_data is not None:
                # area is in m², so multiply to get total emissions per pixel in Mg/year
                total_emission_per_pixel = aggregated_array_mg_per_m2 * self.area_data
            else:
                # If no area data, assume uniform area
            ...
            # Note: total_emission_per_pixel is now in Mg/year (Megagrams per year)
    ...
    def calculate_weighted_values_raster_first(self, raster_data, ipcc_code):
        ...
        for i in range(num_non_zero):
            ...
            if intersecting_cells:
                ...
                if total_intersection_area > 0:
                    ...
                    # Entire pixel value was distributed to DGGS cells
        ...
        # Scale distributed DGGS values to match original raster total of intersecting pixels (per variable)
    ...
    def process_all_netcdf_files(self):
        ...
        Variables are aggregated by IPCC2006 code before raster conversion.
        """
        start_time = time.time()
        self.log_message(f"Processing all {len(self.netcdf_files)} NetCDF files with variable aggregation...")
        ...
        for file_index, netcdf_filename in enumerate(self.netcdf_files):
            ...
            try:
                # Convert NetCDF to aggregated raster data
                ...
                if len(raster_data) > 1:
                    ...
                    # Add all IPCC2006 codes to result dataframe
                    for ipcc_code, weighted_values in weighted_values_dict.items():
                        # Use clean IPCC2006 code name (no file suffix needed since codes are unique)
        ...
        self.log_message(f"\n{'='*60}")
        self.log_message(f"ALL FILES PROCESSED - FINALIZING OUTPUT")
        self.log_message(f"{'='*60}")
        
        # Add Year column with value 2015 for Mexico data
        result_df['Year'] = 2015
        
        # Step 1: Set small values (< 1e-6 Mg = 1g) to zero
        value_columns = [col for col in result_df.columns if col not in ['dggsID', 'Year']]
        ...
        # Step 2: Remove rows where all values are 0 (except dggsID and Year)
        ...
        # Save to single CSV
        output_filename = "Mexico_DGGS_methane_emissions_ALL_FILES.csv"
        ...
        self.log_message(f"Final columns: {len(result_df.columns)}")
        self.log_message(f"Final DGGS cells with data: {len(result_df)}")
