...
class CMSNetCDFToDGGSConverterAggregated:
    """
    Convert CMS NetCDF files to DGGS grid values using aggregated raster-first processing.
    ...
    def __init__(self, netcdf_folder, canada_geojson_path, mexico_geojson_path, output_folder, area_file_path, num_cores=None):
        """
        Initialize the converter.
        
        Args:
            netcdf_folder (str): Path to folder containing CMS NetCDF files
            canada_geojson_path (str): Path to the Canada DGGS GeoJSON file
            mexico_geojson_path (str): Path to the Mexico DGGS GeoJSON file
        ...
        self.canada_geojson_path = canada_geojson_path
        self.mexico_geojson_path = mexico_geojson_path
        ...
        # Load the DGGS grids
        self.log_message("Loading DGGS grids...")
        self.canada_dggs_grid = gpd.read_file(canada_geojson_path)
        self.mexico_dggs_grid = gpd.read_file(mexico_geojson_path)
        self.log_message(f"Loaded {len(self.canada_dggs_grid)} Canada DGGS cells")
        self.log_message(f"Loaded {len(self.mexico_dggs_grid)} Mexico DGGS cells")
        
        # Check if zoneID column exists
        if 'zoneID' not in self.canada_dggs_grid.columns:
            raise ValueError("Canada GeoJSON file must contain 'zoneID' column")
        if 'zoneID' not in self.mexico_dggs_grid.columns:
            raise ValueError("Mexico GeoJSON file must contain 'zoneID' column")
        ...
        # Create spatial indices for DGGS cells for fast intersection queries
        self.log_message("Creating spatial indices for DGGS cells...")
    ...
    def _setup_logging(self):
        ...
        log_filename = f"cms_netcdf_to_dggs_conversion_{timestamp}.log"
        log_path = os.path.join(log_folder, log_filename)
        
        # Create a new logger specifically for this script
        self.logger = logging.getLogger(f"cms_converter_{timestamp}")
    ...
    def _load_ipcc_lookup(self):
        """Load the IPCC2006 variable lookup table."""
        lookup_path = "data/lookup/cms_netcdf_variable_lookup.csv"
    ...
    def _load_area_data(self):
        ...
        try:
            ...
            # Save full area data
            area_output_path = os.path.join(area_npy_folder, "cms_area_data_full.npy")
    ...
    def _compute_cell_area_cm2(self, lat_centers, lon_centers, cell_deg=0.1):
        """Compute cell areas (cm^2) for a regular lon/lat grid using spherical Earth."""
        R = 6371000.0  # meters
        half = cell_deg / 2.0
    ...
    def _get_area_for_dimensions(self, target_lat, target_lon, area_lat, area_lon, area_data):
        ...
        if missing_lat or missing_lon:
            ...
            target_area = self._compute_cell_area_cm2(target_lat, target_lon)
        else:
            ...
            # Convert m^2 to cm^2 to match flux units cm^-2
            target_area = target_area_m2 * 1.0e4
    ...
    def _create_spatial_indices(self):
        """Create spatial indices for fast DGGS cell intersection queries."""
        # Create bounding boxes for Canada DGGS cells
        self.canada_dggs_bounds = []
        for idx, row in self.canada_dggs_grid.iterrows():
            geom = row.geometry
            bounds = geom.bounds  # (minx, miny, maxx, maxy)
            self.canada_dggs_bounds.append((bounds, idx))
        
        # Create bounding boxes for Mexico DGGS cells
        self.mexico_dggs_bounds = []
        for idx, row in self.mexico_dggs_grid.iterrows():
            geom = row.geometry
            bounds = geom.bounds  # (minx, miny, maxx, maxy)
            self.mexico_dggs_bounds.append((bounds, idx))
        
        self.log_message(f"  Created spatial index for {len(self.canada_dggs_bounds)} Canada DGGS cells")
        self.log_message(f"  Created spatial index for {len(self.mexico_dggs_bounds)} Mexico DGGS cells")
    ...
    def extract_year_from_filename(self, filename):
        ...
        """
        # Extract year from filename like 'CMS_CH4_FLX_CA_2013.nc' or 'CMS_CH4_FLX_MX_2010.nc'
    ...
    def determine_country_from_filename(self, filename):
        """
        Determine country from NetCDF filename.
        
        Args:
            filename (str): NetCDF filename
            
        Returns:
            str: 'Canada' or 'Mexico'
        """
        if 'CA_' in filename or 'Canada' in filename:
            return 'Canada'
        elif 'MX_' in filename or 'Mexico' in filename:
            return 'Mexico'
        else:
            raise ValueError(f"Could not determine country from filename: {filename}")

    def aggregate_variables_by_ipcc_code(self, nc_data, country):
        """
        Aggregate variables by IPCC2006 codes using the lookup table.
        Filters variables based on country prefix.
        
        Args:
            nc_data (xarray.Dataset): NetCDF dataset
            country (str): 'Canada' or 'Mexico'
            
        Returns:
            dict: Dictionary with aggregated data for each IPCC2006 code
        """
        self.log_message(f"Aggregating variables by IPCC2006 codes for {country}...")
        ...
        # Filter variables by country prefix
        country_prefix = 'CA' if country == 'Canada' else 'MX'
        country_variables = [var for var in variables if var.startswith(f'CH4_FLX_{country_prefix}_')]
        
        self.log_message(f"  Found {len(variables)} total variables")
        self.log_message(f"  Found {len(country_variables)} {country} variables")
        ...
        self.log_message(f"  Found {len(ipcc_groups)} unique IPCC2006 codes for {country}:")
    ...
    def convert_aggregated_to_raster(self, netcdf_path, country):
        """
        ...
        """
        self.log_message(f"Processing NetCDF: {os.path.basename(netcdf_path)} for {country}")
        
        # Load NetCDF data
        nc_data = xr.open_dataset(netcdf_path, decode_times=False)
        
        # Aggregate variables by IPCC2006 codes
        aggregated_data = self.aggregate_variables_by_ipcc_code(nc_data, country)
        ...
        if hasattr(self, 'area_lat') and hasattr(self, 'area_lon'):
            area_npy_path = "data/area_npy/cms_area_data_full.npy"
            if os.path.exists(area_npy_path):
                ...
                area = self._compute_cell_area_cm2(lat, lon)
        else:
    ...
    def calculate_weighted_values_raster_first(self, raster_data, ipcc_code, dggs_grid, dggs_bounds):
        """
        ...
        Args:
            ...
            dggs_bounds: DGGS bounds for spatial indexing
        ...
        results = np.zeros(len(dggs_grid))
        ...
        for i in range(num_non_zero):
            ...
            intersecting_cells = self._find_intersecting_cells(pixel_geom, dggs_grid, dggs_bounds)
            ...
            if intersecting_cells:
                ...
                for cell_idx in intersecting_cells:
                    try:
                        ...
                        cell_geometry = dggs_grid.iloc[cell_idx].geometry
    ...
    def _find_intersecting_cells(self, pixel_geom, dggs_grid, dggs_bounds):
        """
        ...
        for bounds, cell_idx in dggs_bounds:
            # Quick bounding box check first
            if self._bounds_intersect(pixel_bounds, bounds):
                # Detailed geometry intersection check
                try:
                    if pixel_geom.intersects(dggs_grid.iloc[cell_idx].geometry):
    ...
    def process_single_netcdf_file(self, netcdf_filename):
        ...
        try:
            # Determine country and year from filename
            country = self.determine_country_from_filename(netcdf_filename)
            year = self.extract_year_from_filename(netcdf_filename)
            
            self.log_message(f"  Country: {country}")
            self.log_message(f"  Year: {year}")
            
            # Select appropriate DGGS grid and bounds
            if country == 'Canada':
                dggs_grid = self.canada_dggs_grid
                dggs_bounds = self.canada_dggs_bounds
            else:  # Mexico
                dggs_grid = self.mexico_dggs_grid
                dggs_bounds = self.mexico_dggs_bounds
            
            # Initialize result dataframe with DGGS cell IDs (zoneID)
            result_df = dggs_grid[['zoneID']].copy()
            ...
            raster_data = self.convert_aggregated_to_raster(netcdf_path, country)
            ...
            for ipcc_code, raster_info in raster_data.items():
                ...
                weighted_values = self.calculate_weighted_values_raster_first(
                    raster_info, ipcc_code, dggs_grid, dggs_bounds
                )
            ...
            # Save to CSV
            output_filename = f"CMS_{country}_DGGS_methane_emissions_{year}.csv"
...
def main():
    """
    Main function to run the CMS NetCDF to DGGS conversion.
    """
    # Configuration - Updated paths for HPC
    netcdf_folder = "/home/mingke.li/GridInventory/2010_2013_CMS_Canada_Mexico_Methane_Flux_oil_gas"
    canada_geojson_path = "data/geojson/global_countries_dggs_merge/Canada_CAN_grid.geojson"
    mexico_geojson_path = "data/geojson/global_countries_dggs_merge/México_MEX_grid.geojson"
    ...
    if not os.path.exists(canada_geojson_path):
        print(f"Error: Canada GeoJSON file not found: {canada_geojson_path}")
        return
    
    if not os.path.exists(mexico_geojson_path):
        print(f"Error: Mexico GeoJSON file not found: {mexico_geojson_path}")
        return
    ...
    converter = CMSNetCDFToDGGSConverterAggregated(
        netcdf_folder, canada_geojson_path, mexico_geojson_path, 
        output_folder, area_file_path
    )
